...
def rdp_bezier(points, T, epsilon, method="middle"):
    ...
    coefficients, T_new = fit_directly_bezier(points[:, 0], points[:, 1], T, False, 10) # Example params
    ...
    if max_idx != -1 and max_dist > epsilon:
        ...
        recursive_left = rdp_bezier(left_points, left_T, epsilon, method)

        right_points = points[max_idx:]
        right_T = T_new[max_idx:]
        recursive_right = rdp_bezier(right_points, right_T, epsilon, method)
